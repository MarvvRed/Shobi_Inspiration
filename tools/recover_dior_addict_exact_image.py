...
DB=ROOT/'database_complete.json'; IMAP=ROOT/'perfume-images'/'map.js'; OUT=ROOT/'dior-addict-image-recovery.json'
...
target=ROOT/'perfume-images'/f'{FID}.avif'; used=''; attempts=[]
for u in CANDIDATES:
...
mp[CODE]=f'perfume-images/{FID}.avif'; IMAP.write_text(prefix+json.dumps(mp,ensure_ascii=False,separators=(',',':'))+';\n',encoding='utf-8')
OUT.write_text(json.dumps({'code':CODE,'fid':FID,'page':PAGE,'source':used,'target':f'perfume-images/{FID}.avif','attempts':attempts},ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
