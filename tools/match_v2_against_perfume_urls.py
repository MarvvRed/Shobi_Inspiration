...
def toks(s): return [x for x in ascii_norm(s).split() if x not in STOP and len(x)>1]
...
def parse_url(u):
    ...
    brand=brand_slug.replace('-',' '); name=name_slug.replace('-',' ')
    full=brand+' '+name
    return {'url':u,'id':fid,'brand':brand,'name':name,'full':full,'tokens':set(toks(full))}

def score(qtokens,c):
    qt=set(qtokens); ct=c['tokens']
    if not qt or not ct: return 0.0,0.0,0.0
    inter=len(qt&ct); union=len(qt|ct)
    jac=inter/union if union else 0.0
    cov=inter/len(qt)
    seq=SequenceMatcher(None,' '.join(qtokens),' '.join(sorted(ct))).ratio()
    return max((jac+cov)/2,seq*0.85),cov,jac

urls=[]; seen=set(); inv=defaultdict(set)
for line in URLS.read_text(encoding='utf-8',errors='ignore').splitlines():
    c=parse_url(line)
    if not c or c['id'] in seen: continue
    idx=len(urls); urls.append(c); seen.add(c['id'])
    for t in c['tokens']: inv[t].add(idx)
...
for p in walk(raw):
    ...
    qtokens=toks((name+' '+brand).strip())
    candidate_idxs=set()
    for t in qtokens: candidate_idxs.update(inv.get(t,set()))
    ranked=[]
    for idx in candidate_idxs:
        c=urls[idx]; sc,cov,jac=score(qtokens,c)
        if sc>=0.42 or cov>=0.60: ranked.append((sc,cov,jac,c))
    ranked.sort(key=lambda x:(x[0],x[1],x[2]),reverse=True)
    top=ranked[:5]
    if not top: cls='NO_CANDIDATE'
    elif top[0][0]>=0.86 and top[0][1]>=0.80 and (len(top)==1 or top[0][0]-top[1][0]>=0.08): cls='STRONG_UNIQUE'
    elif top[0][0]>=0.72 and top[0][1]>=0.70: cls='GOOD_REVIEW'
    else: cls='WEAK_REVIEW'
...
for i in range(1,6): fields += [f'cand{i}_id',f'cand{i}_brand',f'cand{i}_name',f'cand{i}_url',f'cand{i}_score',f'cand{i}_coverage']
