...
def infer_tail_brand(tail):
    ...
    if scores[0][0]>=.90 and margin>=.08:
        return urls[next(iter(brand_idx[scores[0][1]]))]['brand']
    return ''
...
for p in perf:
    ...
    explicit=str(p.get('brand') or '').strip(); prior=brand_by_suffix.get(suffix(code),''); tail_hint=infer_tail_brand(tail) if not explicit and not prior else ''
    hint=explicit or prior or tail_hint; hint_source='field' if explicit else ('verified_suffix' if prior else ('name_tail' if tail_hint else ''))
    brand_pool=set(brand_idx.get(norm(hint),set())) if hint else set(); ids=set()
    for t in toks(qname):ids.update(inv.get(t,set()))
    pool=brand_pool if brand_pool else (ids if ids else range(len(urls)))
    ranked=[]
    for idx in pool:
    ...
    exact_global=list(exact_name_idx.get(norm(qname),set()))
    exact_unique=(len(exact_global)==1 and (not hint or exact_global[0] in brand_pool))
    if exact_unique:
        ei=exact_global[0]
        if not top or top[0][6]['id']!=urls[ei]['id']:
            c=urls[ei]; bs=brand_sim(hint,c); top=[(1.0 if not hint else .78+.22*bs,1.0,bs,1.0,1.0,1.0,c)]+top
            top=top[:5]
    if not top:cls='NO_CANDIDATE'
        ...
        if exact_unique and top[0][6]['id']==urls[exact_global[0]]['id']:cls='STRONG_GLOBAL_UNIQUE'
...
for k in ['STRONG_GLOBAL_UNIQUE','STRONG_UNIQUE','GOOD_REVIEW','WEAK_REVIEW','NO_CANDIDATE']:lines.append(f'- {k}: **{cnt[k]}**')
lines += ['','No Fragrantica web access is used. Matching uses only repository-local `perfume_urls.txt` plus already-verified rows for brand priors. Brand-known rows are searched inside that brand only. A globally exact name is strong only when exactly one local URL has that normalized perfume name. No mapping is promoted automatically.','','## Strong candidates','']
for r in rows:
    if r['classification'] in {'STRONG_GLOBAL_UNIQUE','STRONG_UNIQUE'}:lines.append(f"- `{r['shobi_code']}` — {r['shobi_name']} -> {r.get('cand1_brand','')} / {r.get('cand1_name','')} — ID {r.get('cand1_id','')} — {r['classification']} — brand-source {r['brand_source'] or 'none'}")
