...
out=[]
for r in rows(MATCH):
 if r.get('classification')!='WEAK_REVIEW':continue
 ...
 if best[0]>=0.62:
  out.append({'shobi_code':r.get('shobi_code',''),'shobi_name':r.get('shobi_name',''),'brand':brand,'candidate_id':best[5],'candidate_brand':best[6],'candidate_name':best[7],'rank':best[4],'abbr_score':f'{best[0]:.4f}','seq':f'{best[1]:.4f}','containment':f'{best[2]:.4f}','prefix_coverage':f'{best[3]:.4f}','margin':f'{margin:.4f}','url':best[8]})
out.sort(key=lambda x:(-float(x['abbr_score']),-float(x['margin']),x['shobi_code']))
fields=['shobi_code','shobi_name','brand','candidate_id','candidate_brand','candidate_name','rank','abbr_score','seq','containment','prefix_coverage','margin','url']
with OUTCSV.open('w',encoding='utf-8',newline='') as f:w=csv.DictWriter(f,fieldnames=fields);w.writeheader();w.writerows(out)
lines=['# Brand-locked abbreviation review','',f'- Surfaced candidates: **{len(out)}**','','## Top candidates','']
for x in out[:80]:lines.append(f"- `{x['shobi_code']}` — {x['shobi_name']} -> {x['candidate_brand']} / {x['candidate_name']} — ID {x['candidate_id']} — rank {x['rank']} — score {x['abbr_score']} — margin {x['margin']}")
OUTMD.write_text('\n'.join(lines)+'\n',encoding='utf-8');print('surfaced',len(out))
