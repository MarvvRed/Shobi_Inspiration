...
V=json.loads((ROOT/'social-card-main-notes-validated.json').read_text(encoding='utf-8'))
...
(ROOT/'validated-note-slot-patterns.json').write_text(json.dumps(out,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
...
for k,items in out['byCount'].items():
 ...
 lines.append('')
(ROOT/'validated-note-slot-patterns.md').write_text('\n'.join(lines),encoding='utf-8')
