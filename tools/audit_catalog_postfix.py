...
def name(r): return g(r,'Perfume','perfume','Name','name')
def url(r): return g(r,'Shobi URL','shobi_url','URL','url')
def pid(r): return g(r,'Prestashop ID','prestashop_id','product_id')
def fid(r): return g(r,'Fragrantica ID','fragrantica_id')

def dups(vals):
    c=Counter(v for v in vals if v); return {k:n for k,n in c.items() if n>1}
ident=defaultdict(list)
for r in R:
    ident[(norm(brand(r)),norm(name(r)))].append(code(r))
ident={k:v for k,v in ident.items() if k[0] and k[1] and len(v)>1}
empty=[r for r in R if not code(r)]
conf=[]
for r in R:
    c=code(r); u=url(r)
    m=re.search(r'/(\d+[-_][a-z0-9]+)(?:[-_/]|$)',u,re.I)
    if c and m:
        uc=m.group(1).replace('_','-').upper()
...
lines=['# Post-fix catalog audit','',f'- Rows: **{len(R)}**',f'- Unique non-empty codes: **{len(set(code(r) for r in R if code(r)))}**',f'- Duplicate code groups: **{len(dups([code(r) for r in R]))}**',f'- Duplicate Prestashop ID groups: **{len(dups([pid(r) for r in R]))}**',f'- Duplicate Shobi URL groups: **{len(dups([url(r) for r in R]))}**',f'- Duplicate Fragrantica ID groups: **{len(dups([fid(r) for r in R]))}**',f'- Same normalized brand+name groups: **{len(ident)}**',f'- Empty-code rows: **{len(empty)}**',f'- Code-vs-URL conflicts: **{len(conf)}**','','## Key codes']
for c in checks:
