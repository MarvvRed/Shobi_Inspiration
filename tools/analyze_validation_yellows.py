...
issue_codes = defaultdict(list)
check_codes = defaultdict(list)
...
for r in yellow:
    ...
    for issue in issues:
        ...
        issue_codes[issue].append(code)
    for check in failed:
        check_counts[check] += 1
        check_codes[check].append(code)
    report_rows.append({
        "code": code,
        "brand": r.get("brand"),
        "inspiredBy": r.get("inspiredBy"),
        "fragranticaUrl": r.get("fragranticaUrl"),
        "issues": issues,
        "failedChecks": failed,
        "notesCount": r.get("validationNotesCount", 0),
        "matchedNotesCount": r.get("validationMatchedNotesCount", 0),
        "iconsCount": r.get("validationIconsCount", 0),
    })

payload = {
    "catalogRows": len(rows),
    "yellowRows": len(yellow),
...
print(f"catalog={len(rows)} yellow={len(yellow)}")
print("failed_checks", dict(check_counts))
