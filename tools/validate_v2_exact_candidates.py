...
PILOT=ROOT/'database/audits/label-ocr-pilot.json'
CAT=ROOT/'database/catalog/database_complete.json'
OUT=ROOT/'database/audits/v2-exact-validation.json'
...
def main():
    ...
    for r in pilot.get('rows',[]):
        if r.get('result')!='EXACT_LABEL_SEQUENCE':
            continue
        c=code(r.get('code')); cat=by.get(c)
        reasons=[]
    ...
    payload={'sourceMode':pilot.get('mode'),'exactCandidates':len(rows),'counts':dict(counts),'rule':'Strong exact requires catalog/FID stability, exact ordered equality, and for every label >=2 eligible exact OCR reads including >=1 crop read, with zero competing eligible candidates.','rows':rows}
