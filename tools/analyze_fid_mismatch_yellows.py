...
VALID={str(x.get('code') or '').strip().upper():x for x in json.loads((ROOT/'database/fragrantica/social-cards/records/social-card-main-notes-validated.json').read_text(encoding='utf-8'))}
...
out={'count':len(rows),'rows':rows};(ROOT/'database/audits/fid-mismatch-yellows.json').write_text(json.dumps(out,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
