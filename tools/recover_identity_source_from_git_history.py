...
recovered={}
commits=subprocess.check_output(['git','log','--format=%H','--','database/catalog/database_complete.json'],cwd=ROOT,text=True).splitlines()
# Current commit cannot help; walk newest historical snapshots first.
for sha in commits[1:250]:
    if not remaining: break
    try:
        raw=subprocess.check_output(['git','show',f'{sha}:database/catalog/database_complete.json'],cwd=ROOT)
...
(ROOT/'database/audits/identity-source-history-recovery.json').write_text(json.dumps({'targets':len(targets),'changed':len(changed),'unrecovered':len(remaining),'rows':changed},ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
