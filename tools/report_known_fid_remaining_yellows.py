...
db=json.loads((ROOT/'database_complete.json').read_text(encoding='utf-8-sig'));site=json.loads((ROOT/'catalog_site.json').read_text(encoding='utf-8-sig'))
rows=[]
...
out={'count':len(rows),'rows':rows};(ROOT/'known-fid-remaining-yellows.json').write_text(json.dumps(out,ensure_ascii=False,indent=2)+'\n',encoding='utf-8');print(json.dumps(out,ensure_ascii=False,indent=2))
