...
FID_ONLY={
 '184-YAS':22418,'185-AL HAR':61372,'202-CLIV':4653,
 ...
 # Web-verified final identities
 '904-SALV':34536,'2178-MON':69818,'2552-SOR':75119,'570-DOL':484,
}
CLEAR_FID={'130-LEL'} # Santal 26 is Le Labo home fragrance/candle; no personal-perfume FID.
...
# 612-ESC is Shobi TURQUOISE, distinct from TURQUOISE SUMMER (613-ESC); no exact Fragrantica page found.
# 1526-AL HAR Mukhallat Al Emirates: genuine product, no exact Fragrantica page.
# 1955-LAP Cellular Energizing: body/skincare fragrance, not a standalone Fragrantica perfume page.
# 334-SHI remains source-identity ambiguous; do not coerce to Declare or another Shirley May perfume.
...
def clear_fid(p):
 for k in ('fragranticaId','fragrantica_id','fid'): 
  if k in p:p[k]=None
 if 'fragranticaUrl' in p:p['fragranticaUrl']=None
 if 'fragrantica_url' in p:p['fragrantica_url']=None
def patch(path):
 ...
 for p in rows:
  c=code_of(p)
  if c in CLEAR_FID: clear_fid(p);hit.append(c+' (cleared invalid FID)')
  elif c in FIXES:
