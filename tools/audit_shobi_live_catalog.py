...
CATEGORIES={
 'Fragrances For Women':'https://leparfum.com.gr/en/perfumes/fragrances-for-women',
 'Fragrances For Men':'https://leparfum.com.gr/en/perfumes/fragrances-for-men',
 'Elegants Fragrances':'https://leparfum.com.gr/en/perfumes/elegants-fragrances',
 'Niche Perfumes':'https://leparfum.com.gr/en/perfumes/niche-perfumes',
 'Luxury Perfumes':'https://leparfum.com.gr/en/perfumes/luxury-perfumes',
}
PAGE_SIZE=24
MAX_PAGES=80
...
def get(url, attempts=6):
 ...
 for i in range(attempts):
  try:
   ...
   last=e
   wait=min(30,2**i)+random.uniform(.4,1.4)
...
def scrape_category(category,base_url):
 ...
 for page in range(1,MAX_PAGES+1):
  q=urlencode({'page':page,'resultsPerPage':PAGE_SIZE})
  url=f'{base_url}?{q}'
  html=get(url); rows=parse_products(html,category)
  page_codes=[r['shobi_code'] for r in rows]
  fresh=[r for r in rows if (r['shobi_code'],r['url']) not in seen]
  print(f'{category}: page {page}: parsed={len(rows)} fresh={len(fresh)}',flush=True)
  if not rows: break
  for r in fresh:
   seen.add((r['shobi_code'],r['url'])); out.append(r)
  if not fresh: break
...
def main():
 ...
 for cat,url in CATEGORIES.items():
  rows=scrape_category(cat,url)
  diagnostics.append((cat,len(rows)))
