...
MANUAL={
    '118-HAM':'Hamidi Oud & Perfumes',
    '166-AZ':"The Perfumer's Story by Azzi",
    '171-DUP':'S.T. Dupont',
    '254-JOM':'Jo Malone London',
    '287-JOM':'Jo Malone London',
    '304-KIE':"Kiehl's",
    '307-LOC':"L'Occitane en Provence",
    '309-LOC':"L'Occitane en Provence",
    '325-PECK':"Pecksniff's",
    '327-PECK':"Pecksniff's",
    '523-DRC':'Dior',
    '534-DRC':'Dior',
    '637-EST':'Estée Lauder',
    '992-ZAD':'Zadig & Voltaire',
    '993-ZAD':'Zadig & Voltaire',
    '994-ZAD':'Zadig & Voltaire',
    '1118-FAB':'Brut Parfums Prestige',
    '2016-PARELM':"Parfums d'Elmar",
    'pid:2688':'Ajmal',
}
...
with AUDIT.open(encoding='utf-8-sig', newline='') as f:
    for r in csv.DictReader(f):
        ...
        brand=canonical((r.get('fragrantica_brand') or '').strip())
...
def build_source(rows):
    ...
    for p in rows:
        ...
        if len(slugs)==1:
            sl=next(iter(slugs))
            source[code]=canonical(slug_brand.get(sl) or display_slug(sl))
    ...
    for p in rows:
        code=str(p.get('code') or '')
        if code not in source and suffix(code) in unanimous:
            source[code]=unanimous[suffix(code)]
    return source
...
def apply(path):
    ...
    if missing:
        raise SystemExit(f'{path}: unresolved brand codes: {missing}')
    path.write_text(json.dumps(rows,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
    print(path, 'changed', changed, 'rows', len(rows))
