...
OUT=Path('database/catalog/database_v2_clean.json')
REPORT=Path('database/audits/database-v2-migration-status.md')
...
REPORT.write_text(f'''# Clean site database v2 migration\n\n- Master source: `database/archive/catalog/shobi-master-v2-2369.csv`\n- Clean rows created: **{len(out)}**\n- Unique non-empty Shobi codes: **{len(unique_codes)}**\n- Legitimate empty-code rows: **{empty}**\n- Fragrantica mappings automatically trusted from old DB: **0**\n- Gender automatically trusted from old DB: **0**\n- Season automatically trusted from old DB: **0**\n- Notes automatically trusted from old DB: **0**\n- Status: **ENRICHMENT IN PROGRESS**\n\n`database/catalog/database_v2_clean.json` is intentionally rebuilt from the canonical Shobi master. Old enrichment is not copied into the clean database. Existing historical/corpus data may be used only as candidate evidence and must be reverified before promotion. The live site remains on `database/catalog/database_complete.json` until the clean database passes the 2369-row enrichment/quality gate.\n''', encoding='utf-8')
