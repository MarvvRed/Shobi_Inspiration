# Current catalog audit

- Catalog rows: **2324**
- Public-site rows: **2324**
- Green rows audited: **2086**
- Green-row errors: **0**
- Yellow (intentionally unverified) rows: **238**
- Red rows: **0**
- DB/site projection mismatches: **0**
- Fragrantica URL/FID mismatches among green rows: **0**
- Duplicate Prestashop product IDs: **0**
- Duplicate Shobi URLs: **0**

## Green proof methods

- `current-fid-exact-card`: **1026**
- `strict-ordered-image`: **1060**

## Confirmed green-row errors

- None.

## Myths Man — Amouage

- `1912-AMG`, FID `38259`: exact 6-note match — Ash, Chrysanthemum, Leather, Orris Root, Elemi, Labdanum.

## Parallel live Shobi listings retained

- `1068-CHA`: distinct product IDs 591, 165.
- `1270-VAN`: distinct product IDs 971, 346.
- `390-ACQ`: distinct product IDs 502, 106.
- `777-LAL`: distinct product IDs 831, 284.

## Intentionally unverified yellow rows

- `2833-AMG` — Amouage — Meander: socialCard, notes.
- `2816-MOOD` — Mood London — Blonde Maracuja Mood London: identity, fid, url, socialCard, image, notes, icons, gender, season.
- `2814-TMFO` — Tom Ford — Figue Erotique: socialCard, notes.
- `2806-PHL` — Phlur — Afterglow: socialCard, notes.
- `2786-LTN` — Louis Vuitton — Rain Tea Louis Vuitton: socialCard, notes.
- `2783-LTN` — Louis Vuitton — Ink Mark Louis Vuitton: socialCard, notes.
- `2778-JOM` — Jo Malone London — Fig Lotus Flower: socialCard, notes.
- `2744-NISH` — Nishane — Meant To Be Seen: socialCard, notes.
- `2763-KAY` — Kayali Fragrances — Freedom Musk Bouquet 27 Eau de Parfum: socialCard, notes.
- `2750-NISH` — Nishane — Shem: socialCard, notes.
- `2743-BORN` — BORNTOSTANDOUT — Black Guava: socialCard, notes.
- `2741-ROJ` — Roja Dove — Lost In Paris: socialCard, notes.
- `2722-CRIVEL` — Maison Crivelli — Hibiscus Mahajad: socialCard, notes.
- `2720-ROJ` — Roja Dove — United Arab Emirates Spirit Of The Union: socialCard, notes.
- `2719-FRAG` — Fragrance Du Bois — New-york-5th-avenue: socialCard, notes.
- `2715-CRD` — Creed — Oud Zarian: socialCard, notes.
- `2712-TMFO` — Tom Ford — Oud Voyager: socialCard, notes.
- `2708-BORN` — BORNTOSTANDOUT — Black Mango: socialCard, notes.
- `2663-ARM` — Giorgio Armani — Emporio Armani Stronger With You Parfum: socialCard, notes.
- `2700-BLG` — Bvlgari — Amunae: socialCard, notes.
- `2701-DIP` — Diptyque — Cafe Verlet: identity, fid, url, socialCard, image, notes, icons, gender, season.
- `2646-PEN` — Penhaligon's — The Dandy: icons.
- `2645-LTN` — Louis Vuitton — eLVes: socialCard, notes.
- `2458-LTN` — Louis Vuitton — LV Lovers: socialCard, notes.
- `2617-DOL` — Dolce & Gabbana — Devotion Pour Homme: socialCard, notes.
- `2622-PHL` — Phlur — Vanilla-skin-eau-de-: socialCard, notes.
- `2649-BORN` — BORNTOSTANDOUT — Naked Laundry: icons.
- `2625-GUR` — Guerlain — L Instant de Guerlain pour Homme Eau Extreme: socialCard, notes.
- `2598-GLO` — Glossier — You: socialCard, notes.
- `2375-ESC` — Escada — Brisa Cubana: socialCard, notes.
- `2562-JUSB` — Jusbox — Beat Cafe: socialCard, notes.
- `2586-DIP` — Diptyque — Biscuit DIP: identity, fid, url, socialCard, image, notes, icons, gender, season.
- `2588-BLG` — Bvlgari — Onekh: socialCard, notes.
- `2525-CAR` — Carolina Herrera — Good Girl Supreme: socialCard, notes.
- `2548-RIT` — Rituals — Hammam: socialCard, notes.
- `2380-CAR` — Carolina Herrera — Very Good Girl Glam: socialCard, notes.
- `2545-MEM` — Memo Paris — Kedu Sesame: socialCard, notes.
- `2536-PRA` — Prada — Prada Paradoxe Virtual Flower: socialCard, notes.
- `2563-TMFO` — Tom Ford — Rose Exposed: socialCard, notes.
- `2550-ARM` — Giorgio Armani — Emporio Armani Stronger With You Tobacco: socialCard, notes.
- `2521-SOL` — Sol de Janeiro — Carioca Crush: socialCard, notes.
- `2519-MNT` — Montale — Starry Nights: socialCard, notes.
- `2493-SERG` — Serge Lutens — Nuit de Cellophane: socialCard, notes.
- `2492-ETLR` — Etat Libre d Orange — You Or Someone Like You: socialCard, notes.
- `2514-DRC` — Dior — Sauvage Eau Forte: socialCard, notes.
- `2509-CRIVEL` — Maison Crivelli — Oud Maracuja: socialCard, notes.
- `2473-HER` — Hermès — Barenia: socialCard, notes.
- `2468-ESPAR` — Essential Parfums — Bois Imperial: socialCard, notes.
- `2463-GUC` — Gucci — Flora Gorgeous Orchid: socialCard, notes.
- `2452-JOM` — Jo Malone London — Cypress Grapevine Cologne Intense: socialCard, notes.
- `2415-ESC` — Escada — SEXY GRAFFITY - ESCADA: socialCard, notes.
- `2393-KAY` — Kayali Fragrances — Eden Sparkling Lychee 39 Eau de Parfum: socialCard, notes.
- `2386-SOL` — Sol de Janeiro — Cheirosa 68: socialCard, notes.
- `2347-AMG` — Amouage — Portrayal Man: socialCard, notes.
- `2363-VER` — Versace — Santal Boise: socialCard, notes.
- `2366-DIP` — Diptyque — Tam Dao Eau de Toilette: socialCard, notes.
- `2326-TMFO` — Tom Ford — Amber Absolute: socialCard, notes.
- `2319-LTN` — Louis Vuitton — Orage: socialCard, notes.
- `2313-DRC` — Dior — Sauvage Parfum: socialCard, notes.
- `2307-AQU` — Aquolina — Gold Sugar: socialCard, notes.
- `2296-KAY` — Kayali Fragrances — Oudgasm Café Oud 19 Eau de Parfum Intense: socialCard, notes.
- `2271-ROJ` — Roja Dove — Elysium Pour Homme Eau Intense: socialCard, notes.
- `1925-PEN` — Penhaligon's — Babylon: socialCard, notes.
- `2282-DRC` — Dior — Dioriviera Eau de Parfum: socialCard, notes.
- `2265-KAY` — Kayali Fragrances — Oudgasm Rose Oud 16 Eau de Parfum Intense: socialCard, notes.
- `2249-BYR` — Byredo — Rouge Chaotique: socialCard, notes.
- `2229-ESCE` — Escentric Molecules — Molecule 01 + Patchouli: socialCard, notes.
- `2228-ESCE` — Escentric Molecules — Molecule 01 + Iris: socialCard, notes.
- `2223-AMG` — Amouage — Reflection 45 Man: socialCard, notes.
- `2226-BON` — Bond No 9 — The Scent of Peace for Him: socialCard, notes.
- `2213-AMG` — Amouage — Opus XIV Royal Tobacco: socialCard, notes.
- `2169-ROJ` — Roja Dove — Ti Amo: socialCard, notes.
- `2194-ORT` — Orto Parisi — ORTO: socialCard, image, notes, season.
- `2196-BYR` — Byredo — Open Sky: socialCard, notes.
- `2165-KAY` — Kayali Fragrances — Vanilla Royale Sugared Patchouli 64 Eau De Parfum Intense: socialCard, notes.
- `2175-ESCE` — Escentric Molecules — Molecule 01 + Mandarin: socialCard, notes.
- `2159-CAR` — Carolina Herrera — Call Me Darling: icons.
- `2057-ESCE` — Escentric Molecules — Molecule 01 + Black Tea: socialCard, notes.
- `2143-XER` — Xerjoff — Amber Star: socialCard, notes.
- `2141-LTN` — Louis Vuitton — Stellar Times: socialCard, notes.
- `2144-ARM` — Giorgio Armani — Stronger With You Limited Edition: socialCard, notes.
- `2045-HER` — Hermès — Eau d Orange Verte: socialCard, notes.
- `2118-GUC` — Gucci — Gucci Bloom Intense: socialCard, notes.
- `2121-CAR` — Carolina Herrera — Mad World: socialCard, notes.
- `2032-CAR` — Carolina Herrera — True Oud: icons.
- `2099-MARG` — Maison Martin Margiela — Autumn Vibes: socialCard, notes.
- `2106-MARC` — Marc-Antoine Barrois — B683 Eau de Parfum: socialCard, notes.
- `2086-VICT` — Victoria's Secret — Bare: socialCard, notes.
- `2065-FRA` — Franck Boclet — Sugar: socialCard, notes.
- `2085-CLIV` — Clive Christian — X NEROLI LIMITED EDITION - CLIVE CHRISTIAN: identity, fid, url, socialCard, image, notes, icons, gender, season.
- `2029-GUC` — Gucci — Tears From The Moon: socialCard, notes.
- `2020-VICT` — Victoria's Secret — Velvet Petals Sunkissed: socialCard, notes.
- `1978-ARM` — Giorgio Armani — Emporio Armani Stronger With You Intensely: socialCard, notes.
- `1997-LTN` — Louis Vuitton — On The Beach: socialCard, notes.
- `1996-LTN` — Louis Vuitton — Heures d’Absence (2020): socialCard, notes.
- `1929-BLG` — Bvlgari — Le-gemme-tygar-eau-de-: socialCard, notes.
- `1995-YZLO` — Yves Saint Laurent — Caban: socialCard, notes.
- `1968-DOL` — Dolce & Gabbana — LIGHT BLUE ITALIAN LOVE W: socialCard, notes.
- `1938-YZLO` — Yves Saint Laurent — Babycat: socialCard, notes.
- `1950-SWISA` — Swiss Arabian — Al Amaken: socialCard, notes.
- `1899-ZARK` — ZARKOPERFUME — The-muse: socialCard, notes.
- `1878-KIL` — By Kilian — A Kiss from a Rose: socialCard, notes.
- `1890-LEL` — Le Labo — Lys 41: socialCard, notes.
- `1860-NAS` — Nasomatto — Absinth: socialCard, notes.
- `1851-ARM` — Giorgio Armani — Jasmin Kusamono: socialCard, notes.
- `1856-PARF` — Parfums de Marly — Meliora: socialCard, notes.
- `1817-JUL` — Juliette Has a Gun — Sunny Side Up: socialCard, notes.
- `1812-ALJA` — Al Jazeera Perfumes — Palace: socialCard, notes.
- `1803-DRC` — Dior — J Adore Infinissime: socialCard, notes.
- `1780-RCAV` — Roberto Cavalli — Just Cavalli for Her: socialCard, notes.
- `1772-TER` — Tiziana Terenzi — Burdel: socialCard, notes.
- `1764-GUR` — Guerlain — Aqua Allegoria Coconut Fizz: socialCard, notes.
- `1751-GUL` — Jean Paul Gaultier — Madame: socialCard, image, notes, season.
- `1757-DIP` — Diptyque — Eau Duelle Eau de Parfum: socialCard, notes.
- `1748-BYR` — Byredo — Marijuana: identity, fid, url, socialCard, image, notes, icons, gender, season.
- `1746-XER` — Xerjoff — Alexandria III: icons.
- `1731-VICT` — Victoria's Secret — Tease Creme Cloud: icons.
- `1718-TER` — Tiziana Terenzi — Cassiopea: socialCard, notes.
- `1693-BYR` — Byredo — Mixed Emotions: socialCard, notes.
- `1659-CHRA` — Christina Aguilera — Christina Aguilera by Night: socialCard, notes.
- `1655-JOM` — Jo Malone London — Oat Cornflower: socialCard, notes.
- `1660-ESCE` — Escentric Molecules — Molecule 01: socialCard, notes.
- `1639-TER` — Tiziana Terenzi — Casanova: socialCard, notes.
- `1550-YAN` — Yankee Candle — HOME SWEET HOME - YANKEE: identity, fid, url, socialCard, image, notes, icons, gender, season.
- `1615-FRE` — Frédéric Malle — Promise: socialCard, notes.
- `323-NAS` — Nasomatto — Duro: socialCard, notes.
- `1551-VICT` — Victoria's Secret — Tease-dreamer-eau-de-: socialCard, notes.
- `161-PEN` — Penhaligon's — Cairo: socialCard, notes.
- `1003-ACQ` — Acqua di Parma — Colonia Club: socialCard, notes.
- `165-PEN` — Penhaligon's — The Tragedy of Lord George: socialCard, notes.
- `112-BON` — Bond No 9 — Bleecker Street: socialCard, notes.
- `143-MAN` — Mancera — Red Tobacco: socialCard, notes.
- `383-YAN` — Yankee Candle — WEDDING DAY - YANKEE: identity, fid, url, socialCard, image, notes, icons, gender, season.
- `172-SHALM` — Scent Salim — Moroccan-musk: socialCard, notes.
- `283-JOM` — Jo Malone London — Poppy & Barley: socialCard, notes.
- `130-LEL` — Le Labo — SANTAL 26 - LE LABO: identity, fid, url, socialCard, image, notes, icons, gender, season.
- `1097-DOL` — Dolce & Gabbana — Light Blue Discover Vulcano Pour Homme: socialCard, notes.
- `564-DOL` — Dolce & Gabbana — SEXY CHOCLATE: identity, fid, url, socialCard, image, notes, icons, gender, season.
- `589-ELI` — Elie Saab — Girl of Now: socialCard, notes.
- `967-VICT` — Victoria's Secret — Velvet Petals: socialCard, notes.
- `1485-TMU` — Mugler — Alien Man: socialCard, notes.
- `884-RAL` — Ralph Lauren — Big Pony 2: socialCard, image, notes, season.
- `314-MOL` — Molton Brown — ZIAO JAO - MOLTON BROWN: identity, fid, url, socialCard, image, notes, icons, gender, season.
- `1037-BLG` — Bvlgari — Bvl Man: socialCard, image, notes, season.
- `1006-ACQ` — Acqua di Parma — COLONIA INTENSA OUD: socialCard, notes.
- `337-TIFF` — Tiffany Co — Tiffany  Co Sheer: image, season.
- `699-HER` — Hermès — Eau des Merveilles Bleue: socialCard, notes.
- `118-HAM` — Hamidi Oud Perfumes — REHAN - HAMIDI OUD: socialCard, notes, icons, gender, season.
- `382-YAN` — Yankee Candle — POMEGRANATE CIDER - YANKEE CANDLES: identity, fid, url, socialCard, image, notes, icons, gender, season.
- `381-YAN` — Yankee Candle — FRESH CUT ROSES - YANKEE CANDLES: identity, fid, url, socialCard, image, notes, icons, gender, season.
- `380-YAN` — Yankee Candle — FALL FESTIVAL - YANKEE CANDLES: identity, fid, url, socialCard, image, notes, icons, gender, season.
- `379-YAN` — Yankee Candle — CLEAN COTTON - YANKEE CANDLES: identity, fid, url, socialCard, image, notes, icons, gender, season.
- `365-TMFO` — Tom Ford — Patchouli Absolu: socialCard, notes.
- `338-TMFO` — Tom Ford — Arabian Wood: socialCard, notes.
- `336-THCO` — The White Company — NARCISSUS NOIR - THE WHITE COMPANY: identity, fid, url, socialCard, image, notes, icons, gender, season.
- `335-THCO` — The White Company — CASHMERE BLANC - THE WHITE COMPANY: identity, fid, url, socialCard, image, notes, icons, gender, season.
- `334-SHI` — SHIRLEY MAY — SHIRLEY MAY: identity, fid, url, socialCard, image, notes, icons, gender, season.
- `331-PG` — P&G — CRYSTAL WATER & WATER LILY: identity, fid, url, socialCard, image, notes, icons, gender, season.
- `330-PG` — P&G — ORIENTAL BLOSSOM: identity, fid, url, socialCard, image, notes, icons, gender, season.
- `326-PECK` — Pecksniff's — CALM - PECKSNIFF'S: identity, fid, url, socialCard, image, notes, icons, gender, season.
- `325-PECK` — Pecksniff's — BLACK OAK & CEDAR - PECKSNIFF'S: socialCard, notes, icons, gender, season.
- `313-MOL` — Molton Brown — thai vert-mol: identity, fid, url, socialCard, image, notes, icons, gender, season.
- `311-MOL` — Molton Brown — amber cocoon-mol: identity, fid, url, socialCard, image, notes, icons, gender, season.
- `301-JOM` — Jo Malone London — Wild Fig & Cassis: socialCard, notes.
- `289-JOM` — Jo Malone London — SWEET ALMON & MACARRON - JO MALONE: identity, fid, url, socialCard, image, notes, icons, gender, season.
- `262-JOM` — Jo Malone London — Iris Lady Moore: socialCard, notes.
- `261-JOM` — Jo Malone London — INCENSE & EMBERS - JO MALONE: identity, fid, url, socialCard, image, notes, icons, gender, season.
- `258-JOM` — Jo Malone London — GREEN TOMATO LEAF - JO MALONE: identity, fid, url, socialCard, image, notes, icons, gender, season.
- `256-JOM` — Jo Malone London — Grapefruit: socialCard, notes.
- `255-JOM` — Jo Malone London — Golden Needle Tea: socialCard, notes.
- `236-IND` — Indigo Wild — Zum Mist Frankincense Myrrh: socialCard, notes.
- `235-HOLL` — Hollister — Socal: socialCard, notes, icons, gender, season.
- `234-ESCE` — Escentric Molecules — Escentric 04: socialCard, notes.
- `230-DIP` — Diptyque — OPOPANAX: identity, fid, url, socialCard, image, notes, icons, gender, season.
- `228-DIP` — Diptyque — FEUILLE DE LAVANDE: identity, fid, url, socialCard, image, notes, icons, gender, season.
- `227-DIP` — Diptyque — FEU DE BOIS / WOOD FIRE: identity, fid, url, socialCard, image, notes, icons, gender, season.
- `225-DIP` — Diptyque — EL EAU DES SENS: socialCard, notes.
- `223-DIP` — Diptyque — BAIES: identity, fid, url, socialCard, image, notes, icons, gender, season.
- `222-DIP` — Diptyque — AMBRE: identity, fid, url, socialCard, image, notes, icons, gender, season.
- `200-CIR` — CIRE TRUDON — ERNESTO CHE GUEVARA: identity, fid, url, socialCard, image, notes, icons, gender, season.
- `960-VICT` — Victoria's Secret — LOVE ME - VICTORIA'S SECRET: socialCard, notes.
- `953-VICT` — Victoria's Secret — Bombshell Forever: socialCard, notes.
- `928-TRU` — Trussardi — Delicate Rose: socialCard, image, notes, season.
- `898-ROC` — Rochas — AQUA WOMAN - ROCHAS: socialCard, notes.
- `874-PRA` — Prada — Prada Candy Gloss: socialCard, notes.
- `865-PAC` — Rabanne — Pure XS For Her: socialCard, notes.
- `827-MISN` — Missoni — Missoni: image, season.
- `817-MIC` — Michael Kors — Gold Collection  24k Brilliant Gold: socialCard, notes.
- `798-LAN` — Lancôme — Tresor Midnight Rose: socialCard, notes.
- `787-LAN` — Lancôme — Magie Noire: socialCard, notes.
- `783-LAN` — Lancôme — La Vie Est Belle L Eau de Toilette Florale: socialCard, notes.
- `732-JIM` — Jimmy Choo — Flash: socialCard, notes.
- `716-ISS` — Issey Miyake — A Scent by Issey Miyake: socialCard, notes.
- `696-GLA` — Guy Laroche — fidji: socialCard, notes.
- `657-ARM` — Giorgio Armani — Si Intense: socialCard, notes.
- `655-ARM` — Giorgio Armani — Si: socialCard, notes.
- `631-EST` — Estée Lauder — Private Collection: socialCard, notes.
- `630-EST` — Estée Lauder — Pleasures Delight: socialCard, notes.
- `604-ESC` — Escada — Escada Magnetism: socialCard, notes.
- `590-ELIZ` — Elizabeth Arden — 5th Avenue: socialCard, notes.
- `578-DON` — Donna Karan — DKNY Women: socialCard, notes.
- `548-CLI` — Clinique — Happy: socialCard, image, notes, season.
- `515-CHL` — Chloe — Nomade: socialCard, notes.
- `514-CHL` — Chloe — Love Story: socialCard, notes.
- `508-CHA` — Chanel — Coco Noir: socialCard, notes.
- `489-CRT` — Cartier — La Panthere: socialCard, notes.
- `487-CRT` — Cartier — Baiser Vole: socialCard, notes.
- `480-CAR` — Carolina Herrera — 212 Sexy: socialCard, notes.
- `467-CAL` — Calvin Klein — Contradiction: socialCard, notes.
- `465-CAL` — Calvin Klein — Beauty: socialCard, notes.
- `440-BLG` — Bvlgari — Goldea The Roman Night: socialCard, notes.
- `431-BRB` — Burberry — BURBERRY TOUCH: socialCard, notes.
- `418-BRI` — Britney Spears — Fantasy: socialCard, notes.
- `413-BOT` — Bottega Veneta — Bottega Veneta: socialCard, notes.
- `411-BOD` — The Body Shop — White Musk: socialCard, notes.
- `406-BAL` — Balenciaga — Rosabotanica: socialCard, notes.
- `399-ARA` — Aramis — Tuscany Per Donna: socialCard, notes.
- `394-AGE` — Agent Provocateur — Aphrodisiaque: socialCard, notes.
- `393-AER` — Aerin — Lilac Path: socialCard, notes.
- `1289-YZLO` — Yves Saint Laurent — Y: socialCard, notes.
- `1269-VAL` — Valentino — Valentino Uomo Edition Noire: socialCard, notes.
- `1258-SFER` — Salvatore Ferragamo — White Mimosa: socialCard, notes.
- `1251-ROM` — Romane — Royal Blue: identity, socialCard, notes, icons, gender, season.
- `1245-PRA` — Prada — PRADA AMBER: socialCard, image, notes, season.
- `769-LART` — LArtisan Parfumeur — Premier Figuier: socialCard, notes.
- `1191-KEN` — Kenzo — L'Eau par Kenzo Electric Wave pour Homme: socialCard, notes.
- `1172-ISS` — Issey Miyake — L'Eau d'Issey Pour Homme: socialCard, image, notes, season.
- `1160-HUG` — Hugo Boss — Boss Bottled: socialCard, notes.
- `1154-HER` — Hermès — Un Jardin en Mediterranee: socialCard, notes.
- `1102-DAV` — Davidoff — Cool Water: socialCard, notes.
- `1086-CLI` — Clinique — Happy: socialCard, image, notes, season.
- `1077-DRC` — Dior — Fahrenheit 32: socialCard, notes.
- `1067-CHA` — Chanel — Bleu de Chanel: socialCard, notes.
- `1059-CRT` — Cartier — Declaration: socialCard, notes.
- `1043-CAL` — Calvin Klein — Calvin Klein: socialCard, image, notes, season.
- `1036-BLG` — Bvlgari — BLV Pour Homme: socialCard, notes.
- `1020-AZZ` — Azzaro — Wanted: socialCard, notes.
- `1007-ACQ` — Acqua di Parma — Colonia Leather Eau de Cologne Concentree: socialCard, notes.
