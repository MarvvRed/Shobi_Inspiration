...
Archived copy of the resolver used by the Fragrantica workflow.
"""
...
UA = os.environ.get("SHOBI_USER_AGENT", "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126 Safari/537.36")
...
BAD_INSPIRED = {"", "the fragrance notes", "the fragrance notes of", "fragrance notes", "the notes", "notes"}
STOPWORDS = {"for", "the", "and", "of", "by", "pour", "eau", "de", "parfum", "perfume", "fragrance", "edp", "edt", "men", "women", "unisex"}
...
def parse_fragrantica_url(url: str):
    ...
    return {"url": f"https://www.fragrantica.com{p.path}", "id": fid, "brand_slug": parts[1].replace("-", " "), "perfume_slug": perfume_slug.replace("-", " ")}
...
def clean_slug(s: str) -> str:
    ...
    for i, w in enumerate(words):
        out.append(w.lower() if i and w.lower() in small else w[:1].upper() + w[1:])
    return " ".join(out)


def ambiguous(row, note: str):
    return {"prestashop_product_id": str(row.get("prestashop_product_id", "")), "shobi_code": row.get("shobi_code", ""), "inspired_by": row.get("inspired_by", ""), "original_brand": "", "original_perfume": "", "identity_status": "AMBIGUOUS", "fragrantica_status": "NOT_FOUND", "fragrantica_id": "", "fragrantica_url": "", "evidence_note": note[:1800]}
...
def resolve(row):
    ...
    queries = [f'site:fragrantica.com/perfume "{inspired}"', f'Fragrantica "{inspired}"']
    ...
    strong_search = best["coverage"] >= 0.78 and best["score"] >= 0.68 and (second < 0.66 or margin >= 0.10)
    if brand_hint:
        strong_search = strong_search and best["brand_cov"] >= 0.50
    if not strong_search:
        return ambiguous(row, f"online candidate not unique/strong enough; providers={'|'.join(providers)}; brand_hint={brand_hint or 'none'}; score={best['score']:.3f}; coverage={best['coverage']:.3f}; second={second:.3f}")
    ...
    evidence = f"Shobi inspired_by + public web Fragrantica candidate; providers={'|'.join(providers)}; score={best['score']:.3f}; coverage={best['coverage']:.3f}; margin={margin:.3f}"
    if brand_hint:
    ...
    return {"prestashop_product_id": str(row.get("prestashop_product_id", "")), "shobi_code": row.get("shobi_code", ""), "inspired_by": inspired, "original_brand": clean_slug(best["brand_slug"]), "original_perfume": clean_slug(best["perfume_slug"]), "identity_status": "CONFIRMED", "fragrantica_status": "FOUND", "fragrantica_id": best["id"], "fragrantica_url": best["url"], "evidence_note": evidence}
