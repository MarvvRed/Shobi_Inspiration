...
web={r.get('shobi_code','').strip():r for r in read(WEB) if r.get('shobi_code','').strip()}
...
for row in rows:
    ...
    if not m:continue
    # web-confirmed is deliberately curated, and supersedes unresolved/ambiguous local matching.
...
summary['web_confirmed_rows']=len(web);summary['web_confirmed_newly_merged']=merged;summary['web_confirmed_previous_statuses']=dict(previous)
...
print(json.dumps({'web_rows':len(web),'newly_merged':merged,'found_total':summary['found_total'],'ambiguous':summary['ambiguous_total'],'not_found':summary['not_found_total'],'mapped_not_in_corpus':summary['mapped_not_in_corpus_total']},indent=2))
